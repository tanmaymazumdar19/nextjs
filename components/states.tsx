...
export default function States(props: StatesProps): JSX.Element {
  const stateStyle = getStateStyle(props.state)
  
  const StateIcon = getStateIcon(props.state)
  
  return (
    <div className={clsx(stateStyle, 'py-[3px] w-fit px-1 flex gap-1 items-center border rounded text-xs h-[20px] capitalize')}>
      {props.label}
      <StateIcon />
    </div>
  )
